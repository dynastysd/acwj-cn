...
// 解析零个或多个逗号分隔的表达式并返回
// 由 A_GLUE 节点组成的 AST，左子节点是
// 之前表达式的子树（或者 NULL），右子节点是
// 下一个表达式。每个 A_GLUE 节点的 size 字段
// 将被设置为此时树中表达式的数量。如果没有
// 解析到表达式，则返回 NULL
