...
// 给定一个标量类型、一个现有的内存偏移量
// (尚未分配给任何内容)和一个方向(1为向上,-1为向下),计算
// 并返回适合该标量类型的对齐内存偏移量
// 这可能是原始偏移量,也可能在原始偏移量的上方或下方
int cgalign(int type, int offset, int direction) {
  int alignment;

  // 在 x86-64 上我们不需要这样做,但还是让字符在任何偏移处对齐,
  // 并将整数/指针对齐到4字节边界
  switch(type) {
  ...
  // 这里我们有一个整数或长整数,将其对齐到4字节偏移
  // 我在这里放置了通用代码,以便在其他地方重用
  alignment= 4;
...
#define FIRSTPARAMREG 9		// 第一个参数寄存器在寄存器列表中的位置
...
// 给定包含参数值的寄存器,
// 将此参数复制到第 argposn 个参数位置,为将来的函数调用做准备
// 注意,argposn 是 1、2、3、4、...,永远不会为零
void cgcopyarg(int r, int argposn) {

  // 如果超过第六个参数,只需将寄存器压入栈
  // 我们依赖按顺序连续调用参数,对于 x86-64 是正确的
  if (argposn > 6) {
