...
// 左子节点是之前表达式的子树（或 NULL），右子节点是下一个表达式。
// 每个 A_GLUE 节点的 size 字段设置为此时树中的表达式数量。
// 如果没有解析到表达式，则返回 NULL
...
// 解析结构体或联合的成员引用并返回其 AST 树。
// 如果 withpointer 为 true，则通过指针访问成员。
