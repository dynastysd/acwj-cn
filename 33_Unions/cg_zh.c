...
// 给定一个标量类型、一个已存在的内存偏移（尚未分配给任何对象）
// 和一个方向（1 向上，-1 向下），计算并返回
// 该标量类型的适当对齐内存偏移。这可能是原始偏移量，
// 也可能是原始偏移量的上方或下方
int cgalign(int type, int offset, int direction) {
  int alignment;

  // 在 x86-64 上我们不需要做这个，但让字符在任意偏移量对齐，
  // 而整数/指针在 4 字节对齐
  switch(type) {
...
void cgfuncpreamble(struct symtable *sym) {
  ...
  int cnt;
  int paramOffset = 16;		// 任何被推送的参数从该栈偏移量开始
...
// 将整数值转换为布尔值。如果是 IF 或 WHILE 操作则跳转
...
// 给定包含参数值的寄存器，
// 将此参数复制到第 argposn'th 个参数位置，为将来
// 调用函数做准备。注意 argposn 是 1、2、3、4、...，永远不会为零
