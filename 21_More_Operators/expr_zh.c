...
// 解析带有单个表达式参数的函数调用并返回其AST
static struct ASTnode *funccall(void) {
  struct ASTnode *tree;
  int id;

  // 检查标识符是否已定义为函数，然后为其创建叶子节点
...
static struct ASTnode *primary(void) {
  ...
  switch (Token.token) {
    case T_INTLIT:
      // 对于INTLIT token，为其创建一个叶子AST节点
      // 如果在P_CHAR范围内则将其设为P_CHAR类型
...
// 每个token的操作符优先级。必须与defs.h中的token顺序匹配
static int OpPrec[] = {
...
struct ASTnode *binexpr(int ptp) {
  ...
  // 当这个token的优先级高于前一个token的优先级，
  // 或者是右结合的且优先级与前一个token相同
  while ((op_precedence(tokentype) > ptp) ||
	 (rightassoc(tokentype) && op_precedence(tokentype) == ptp)) {
    // 获取下一个整数字面量
    scan(&Token);

    // 递归调用binexpr()并使用我们token的优先级来构建子树
